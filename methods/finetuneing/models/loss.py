...
class NLLLoss(nn.Module):
    ...
    def __init__(self, dist_name='GaussianMixture') -> None:
        super().__init__()
        self.dist = getattr(distributions,dist_name)()

    def forward(self, pred_params, target_obs):
        """Input shape: (N,L)"""
        self.dist.set_params(pred_params)
        losses = -self.dist.log_prob(target_obs)
        return losses