...
class Metrics:
    ...
    _avl_cmat_keys = ("tp", "predp", "possp","all","correct") # hanzr    
    # _avl_cmat_keys = ("tp", "predp", "possp")
    _avl_metrics = ("precision", "recall", "f1", "mean", "std", "mae", "mape", "r2","accuracy") # hanzr
    ...
    def __init__(
        ...
        if set(self._metric_names) & set(("precision", "recall", "f1","accuracy")):#hanzr
    ...
    def compute(
        ...
        # if set(self._metric_names) & set(("precision", "recall", "f1")):
            if self._task in ["ppk", "spk"]:
                ...
                self._data["correct"] = torch.sum(torch.argmax(preds, dim=1) == torch.argmax(targets, dim=1))#hanzr
                self._data["all"] = targets.size(0)  # hanzr           
    ...
    def _update_metric(self, key: str) -> torch.Tensor:
        ...
        elif key == "accuracy":          # hanzr
            v = self._data["accuracy"] = (
                self._data["correct"] / (self._data["all"] + self._epsilon)
            ).mean()
        elif key == "recall":
            v = self._data["recall"] = (
