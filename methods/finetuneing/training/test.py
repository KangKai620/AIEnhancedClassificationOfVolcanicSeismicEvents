...
def test_worker(args, device)->float:
    ...
    with torch.no_grad():
        ...
        for step, (x, loss_targets, metrics_targets, info_for_logging) in enumerate(test_loader):
            ...
            print(f"\n{'='*50}")
            print(f"Batch {step + 1}/{len(test_loader)}")
            ...
            if isinstance(loss_targets, (list, tuple)):
                for i, yi in enumerate(loss_targets):
                    print(f"  loss_targets[{i}].shape: {yi.shape}")
            else:
                print(f"  loss_targets.shape: {loss_targets.shape}")
            
            for key, value in metrics_targets.items():
                if isinstance(value, torch.Tensor):
                    print(f"  metrics_targets['{key}'].shape: {value.shape}")
                else:
                    print(f"  metrics_targets['{key}']: {value}")
            
            print(f"  info_for_logging: {info_for_logging}")
            print(f"{'='*50}")
