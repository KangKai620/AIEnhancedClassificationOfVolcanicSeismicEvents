...
from finetuneing.utils import distributions
...
def process_outputs(
    ...
    for outputs, label_group in zip(outputs_list, label_names):
        ...
        if isinstance(label_group, (tuple, list)):
            ...
            elif label_name == "emg_z" or label_name == 'dis_z':
                distribution = distributions.GaussianMixture()
                distribution.set_params(outputs)
                results[label_name] = distribution.maxprob_val()
                lls = distribution.log_prob(results[label_name])
            else:
